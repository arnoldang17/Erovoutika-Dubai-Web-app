...
document.addEventListener("DOMContentLoaded", () => {
  ...
  nominationForm.addEventListener("submit", async (event) => {
    ...
    const socialMedia = [];
    nominationForm.querySelectorAll('input[name="socialMedia"]:checked').forEach((checkbox) => {
    ...
    try {
      ...
      // Insert Address
      const { error: addressError } = await supabase.from("Address").insert([address]);
      if (addressError) throw new Error(`Address insert failed: ${addressError.message}`);
      console.log("Address inserted");

      // Insert School
      const { error: schoolError } = await supabase.from("School").insert([school]);
      if (schoolError) throw new Error(`School insert failed: ${schoolError.message}`);
      console.log("School inserted");

      // Insert QNA
      const { error: qnaError } = await supabase.from("QNA").insert([qna]);
      if (qnaError) throw new Error(`QNA insert failed: ${qnaError.message}`);
      console.log("QNA inserted");

      // Insert Category
      const { error: categoryError } = await supabase.from("Category").insert([category]);
      if (categoryError) throw new Error(`Category insert failed: ${categoryError.message}`);
      console.log("Category inserted");

      // Insert Designation
      const { error: designationError } = await supabase.from("Designation").insert([designation]);
      if (designationError) throw new Error(`Designation insert failed: ${designationError.message}`);
